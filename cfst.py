...
class CloudflareSpeedTestIStoreOS:
    def __init__(self):
        ...
        parser.add_argument('--full-speed', action='store_true', help="启用完整测速（每个国家独立前10）")
    ...
    def parse_top_ips_by_region(self, csv_path: Path) -> list[str]:
        ...
        # ==================== 完整测速模式：每个国家独立前10 + CSV倒数第二列速度 + 最后一列机场码 ====================
    ...
    def run(self) -> bool:
        print("=" * 80)
        print("🚀 iStoreOS/N1 Cloudflare 测速脚本 [最终版]")
    ...
    def print_summary(self):
        ...
        print(f"模式: {'完整测速' if self.full_speed else '仅延迟测试（不带速度）'}")
